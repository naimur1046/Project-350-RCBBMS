...
export const PatientCrossMatchReport = () => {
...
const navigate = useNavigate();    
  ...
  const inputs = [
    ...
    
   {
    ...
    {
      id: 7,
      name: "patientcrossmatchreportid",
      type: "text",
      placeholder: "Patient Cross Match Report Id",
      errorMessage:
        "Text should be maximum 8 digit",
      label: "patientcrossmatchreportid",
      pattern: "[0-9]{0,8}",
      required: true,
    },
    {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.post('http://localhost:8081/patientcrossmatchreport',values).then((res)=>{
          console.log("Result is"+res)
          navigate('blooddistribution');
      }).then((err)=>{
  ...
  return (
    <div className="bg-gradient-to-r from-[#F28383] from-10% via-[#9D6CD2] via-30% to-[#481EDC] to-90% flex items-center justify-center min-h-screen">
      ...
      <h1 className="text-center text-5xl font-bold text-white">Patient Cross <br/> Match Report</h1>
